...
from .utils import differentiate, edges2Middles, empiricalPDFIntoEmpiricalCDF, renormalizeCDF, renormalizePDF


def getPPFBasedGridFromPPFPoints(bounds: Tuple[NumT, NumT], count: int, dBasedCDFY: ndarray, dBasedCDFX: ndarray, k: int = 3) -> np.ndarray:
...
def genDerivativeBasedPDFFromEmpiricalHistBasedPDF(pdf: Callable, interpX: ndarray) -> Tuple[np.ndarray, np.ndarray]:
