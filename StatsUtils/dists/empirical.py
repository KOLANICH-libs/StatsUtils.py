...
def getEmpiricalPDF(points: np.ndarray, k=3):
	return _getEmpiricalPDFFromSortedPoints(np.sort(points), k=k)


def _getEmpiricalPDFFromSortedPoints(sortedPoints: np.ndarray, k=3):
	cdfX = sortedPoints
	pdfX = edges2Middles(cdfX)
	pdfY = (1 / len(pdfX)) / np.diff(cdfX)
	pdfY = preprocessEmpiricalPDF(pdfY)
	pdfY = renormalizePDF(pdfX, pdfY)
	return pdfX, pdfY


def getEmpiricalCDFPDF(points: np.ndarray, k=3):
	cdfX = np.sort(points)

	cdf = _getEmpiricalCDFFromSortedPoints(cdfX, k=3)
	pdf = _getEmpiricalPDFFromSortedPoints(cdfX, k=3)
	return cdf, pdf
...
class rv_empirical(rv_continuous):
	__slots__ = ("cdfX", "pdfX", "pdfY", "pdfInterp", "cdfInterp", "ppfInterp", "k")

	_support_mask = rv_continuous._support_mask

	def __init__(self, *, points: ndarray = None, cdfX: ndarray = None, cdfY: ndarray = None, pdfX: ndarray = None, pdfY: ndarray = None, pdfInterp: typing.Callable = None, cdfInterp: typing.Callable = None, k=3, **kwargs):
		self.k = k
		self.cdfX = cdfX
		self.pdfX = pdfX
		self.pdfY = pdfY
	...
	def _computePDF(self):
		...
		(self.pdfX, self.pdfY) = _getEmpiricalPDFFromSortedPoints(self.cdfX, k=self.k)
